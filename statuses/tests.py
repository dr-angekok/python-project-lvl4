...
class SratusTest(TestCase):
    ...
    def test_status_view(self):
        ...
        response = self.client.get('/statuses/1/')
        content = response.content.decode("utf-8")
        self.assertTrue(content.__contains__('Any status'))
        response = self.client.post('/statuses/1/update/', {'name': 'Changed status', })
        response = self.client.get('/statuses/')
        content = response.content.decode("utf-8")
        self.assertTrue(content.__contains__('Changed status'))
        self.assertTrue(content.__contains__('"success">Статус успешно изменён<'))
        response = self.client.post('/statuses/1/delete/')
        response = self.client.get('/statuses/')
        content = response.content.decode("utf-8")
        self.assertFalse(content.__contains__('Changed status'))
        self.assertTrue(content.__contains__('"success">Статус успешно удалён<'))
        self.assertTrue(content.__contains__('У вас нет статусов'))
